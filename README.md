...
> 目的：防止接口被公网任意调用而**消耗 DeepSeek 额度**。未开启时不影响本地使用。
...
2. 写入 `backend/.env`：
   ...
   APP_API_KEY=上面生成的那串
   ```
3. 写入 `frontend/.env`（前端根目录新建/编辑）：
   ```
   VITE_API_KEY=同一串
   ```
   前端凭 `VITE_API_KEY` 自动携带 `X-API-Key` 请求头；未配置正确令牌时后端返回 401。

**② 收紧跨域来源（默认已收紧）**

后端默认只放行本地开发地址（`localhost` / `127.0.0.1`）。若前端部署到其它域名，在 `backend/.env` 追加：
```
...
- 用内网穿透（如 cloudflare 隧道）给老板看时，**演示完立刻关闭隧道**，不要长期挂着公网地址。
- **永远不要提交 `backend/.env`**（已在 `.gitignore` 排除）。DeepSeek 密钥只存在后端环境变量中，前端拿不到，也不会进仓库。
